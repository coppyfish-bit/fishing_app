...
def show_analysis_page(df):
    ...
    def process_coords(target_df):
        ...
        def extract_step(ph):
            ...
            if any(x in ph_str for x in ["干潮後", "上げ始め", "上げ初め"]): return 1
            if any(x in ph_str for x in ["満潮前", "上げ終盤"]): return 9
            if any(x in ph_str for x in ["満潮後", "下げ始め", "下げ初め"]): return 1
            if any(x in ph_str for x in ["干潮前", "下げ終盤"]): return 9
            return 5
