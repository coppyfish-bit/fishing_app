...
try:
    ...
    st.error("カスタムモジュールの読み込みに失敗しました。ファイル構成を確認してください。")
...
def get_geotagging(exif):
    ...
    if not gps_info: return None
    return {
        'GPSLatitudeRef': gps_info.get(1),
        'GPSLatitude': gps_info.get(2),
        'GPSLongitudeRef': gps_info.get(3),
        'GPSLongitude': gps_info.get(4)
    }
...
with tab1:
    ...
    if uploaded_file:
        ...
        master_places = sorted(df_master['place_name'].unique().tolist())
        place_options = ["自動判定に従う", "（手入力で新規登録）"] + master_places
        selected_place_option = st.selectbox("📍 場所を選択・修正", options=place_options)

        if selected_place_option == "自動判定に従う":
            place_name = st.text_input("場所名を確認/修正", value=st.session_state.detected_place)
            target_group_id = st.session_state.group_id
        elif selected_place_option == "（手入力で新規登録）":
            place_name = st.text_input("新規場所名を入力")
            target_group_id = "default"
        else:
            place_name = selected_place_option
            matched = df_master[df_master['place_name'] == place_name]
            target_group_id = matched['group_id'].iloc[0] if not matched.empty else "default"
            ...
            if not place_name or place_name == "新規地点":
                ...
                try:
                    with st.spinner("📊 潮汐・気象データ解析中..."):
                        ...
                        val_next_h = int((next_h - target_dt).total_seconds() / 60) if next_h else ""
                        val_next_l = int((next_l - target_dt).total_seconds() / 60) if next_l else ""

                        # 3. 画像処理・Cloudinary
                        img_final = Image.open(uploaded_file)
                        # 方向補正
                        try:
                            ex_o = img_final._getexif()
                            if ex_o:
                                orient = next((k for k, v in ExifTags.TAGS.items() if v == 'Orientation'), None)
                                if ex_o.get(orient) == 3: img_final = img_final.rotate(180, expand=True)
                                elif ex_o.get(orient) == 6: img_final = img_final.rotate(270, expand=True)
                                elif ex_o.get(orient) == 8: img_final = img_final.rotate(90, expand=True)
                        except: pass
                        ...
                        img_bytes.seek(0)
                        res_cloud = cloudinary.uploader.upload(img_bytes, folder="fishing_app")

                        # 4. 保存
                        save_data = {
                            "filename": res_cloud.get("secure_url"),
                            ...
                            "次の満潮まで_分": val_next_h, "次の干潮まで_分": val_next_l,
