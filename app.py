...
import pandas as pd
import time
from edit_module import show_edit_page # ★ファイルの一番上でインポート！
...
def get_tide_details(station_code, dt):
    try:
        ...
        # 探すべき日付のパーツ
        ty, tm, td = int(base_dt.strftime('%y')), base_dt.month, base_dt.day
        ...
        for line in lines:
            # 地点記号(HS)が含まれ、かつその直前に正しい「日」と「月」があるか
            # 例: "26 3 1HS" (2026年3月1日 本渡瀬戸)
            pattern = rf"(\d+)\s+({tm})\s+({td}){station_code}"
        ...
        # --- 数値の抽出 ---
        # 行内のすべての数値（マイナス値含む）をリスト化
        # 1-24番目: 毎時潮位, 25-27番目: 年月日, 28番目以降: 満干潮
        nums = re.findall(r'-?\d+', day_data)
        if len(nums) < 27: return None
        ...
        # 2. 満潮・干潮の抽出
        # HS.txtの後半は [時刻, 潮位, 時刻, 潮位...] のペアが並んでいる
        # 地点記号(HS)より後の数値をイベントとして処理
        event_nums = nums[27:] # 年月日地点記号の後のデータ
        event_times = []
        today_ymd = base_dt.strftime('%Y%m%d')

        # 満潮(4ペア分)と干潮(4ペア分)を順番にチェック
        # 満潮：最初の8個 (4ペア)、干潮：次の8個 (4ペア)
        for i in range(0, 8, 2): # 満潮
            if i+1 < len(event_nums):
                t_str, h_val = event_nums[i], event_nums[i+1]
                if t_str != "9999":
                    ev_time = datetime.strptime(today_ymd + t_str.zfill(4), '%Y%m%d%H%M')
                    event_times.append({"time": ev_time, "type": "満潮", "cm": int(h_val)})
        ...
        for i in range(8, 16, 2): # 干潮
            if i+1 < len(event_nums):
                ...
                if t_str != "9999":
                    ev_time = datetime.strptime(today_ymd + t_str.zfill(4), '%Y%m%d%H%M')
                    event_times.append({"time": ev_time, "type": "干潮", "cm": int(h_val)})
