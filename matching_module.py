...
def get_jma_tide_hs():
    """気象庁 suisan/txt フォーマット(136col)を解析"""
    now = datetime.now()
    station_code = "HS"
...
def show_matching_page(df):
    ...
    st.title("🏹 SeaBass Match AI v6.2")
        ...
        if df is not None and not df.empty:
            ...
            for place in df['場所'].unique():
                ...
                if not success_df.empty:
                    ...
                    results.append({'place': place, 'score': total_score, 'hits': len(success_df), 'lure': lure_mode[0] if not lure_mode.empty else "データ不足"})
