...
def show_matching_page(df=None):
    ...
    if st.button("🔄 現在の本渡瀬戸の状況を自動取得"):
        with st.spinner('潮汐・気象データを精密解析中...'):
                        ...
                        for start, e_type in [(80, "満潮"), (108, "干潮")]:
                            for i in range(4):
                                ...
                                if t_raw and t_raw != "9999" and t_raw.isdigit():
                                    events.append({"time": datetime.strptime(d_str + t_raw, '%Y%m%d%H%M'), "type": e_type, "value": int(v_raw)})
            ...
            st.session_state.input_vals.update({'month': now.month, 'tide': tide_val, 'phase': phase_val, 'temp': temp if temp else 15.0, 'rain': rain48, 'wind_dir': w_dir, 'wind_speed': w_speed if w_speed else 0.0})
            st.success(f"解析成功：現在は「{phase_val}」です")

    with st.expander("📝 フィールド状況の微調整", expanded=False):
        ...
        with c3:
            ...
            t_wind_dir = st.selectbox("風向", DIRS_16, index=DIRS_16.index(st.session_state.input_vals['wind_dir']) if st.session_state.input_vals['wind_dir'] in DIRS_16 else 0)
        ...
        if not results.empty:
            for _, row in results.iterrows():
                ...
                with st.expander(f"{label} ({row['マッチ度']}%) ： {row.get('場所', 'ポイント不明')}", expanded=True):
                    ...
                    with col_img:
                        img_url = row.get('filename') # スプレッドシートの画像URL列
