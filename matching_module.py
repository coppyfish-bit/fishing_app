...
def get_jma_tide_hs():
    ...
    try:
        ...
        # 満潮(80-107), 干潮(108-135) を解析
        for start_pos, e_type in [(80, "満潮"), (108, "干潮")]:
            for i in range(4):
                p = start_pos + (i * 7)
                t_str = day_line_part = day_data[p : p+4].strip()
                if t_str and t_str.isdigit() and t_str != "9999":
                    ev_t = datetime.strptime(today_ymd + t_str.zfill(4), '%Y%m%d%H%M')
                    event_times.append({"time": ev_t, "type": e_type})
...
def show_matching_page(df):
    """診断画面UI"""
    st.title("🏹 SeaBass Match AI v6.9")

    if 'm_data' not in st.session_state or st.button("🔄 海況データを手動更新"):
