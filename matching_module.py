    ...
    if df is not None and not df.empty:
        ...
        if not results.empty:
            for _, row in results.iterrows():
                st.write(f"### {row.get('場所', '不明なポイント')} ({row['マッチ度']}%)")
                st.caption(f"実績: {row.get('魚種','-')} {row.get('全長_cm','-')}cm")
